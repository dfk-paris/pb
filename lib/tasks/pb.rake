namespace :pb do
  ...
  desc 'import people from excel'
  task import_people: :environment do
    records = []
    Dir[Rails.root.join('..', 'data', 'people', '*')].each do |f|
      data = Roo::Spreadsheet.open(f).to_a
      headers = data[0]
      records += data[1..-1].map do |r|
        result = {}
        headers.each_with_index do |h, i|
          if h && r[i]
            result[h] = ActionView::Base.full_sanitizer.sanitize(r[i]).strip
          end
        end
        result
      end
    ...
    records.select!{|r| !r['Wikidata ID'].nil? && r['Wikidata ID'] != 'x'}

    mapping = {}
    records.each do |r|
      mapping[r['Name']] = r['Wikidata ID']
    end

    File.open Rails.root.join('config', 'people.json'), 'w' do |outfile|
      outfile.puts mapping.to_json
    end 
  end
